...
var BASE_URL = "/gql";
// const length = organizationModel.find({}).exec();
// console.log("length: ", length);
// await organizationModel.countDocuments().exec())
var length = 0;
describe("Test for queries and mutation", function () {
    ...
    it("Get organizations by id", function (done) {
        request
            .post(BASE_URL)
            .send({
            query: '{ organization(_id:"5f71abbd5936151c29e9419c") { organization, ceo, marketValue} }',
        })
            .expect(200)
            .end(function (err, res) {
    ...
    // 		.send({
    // 			query:
    // 				'mutation {createOrganization(organization:"SkyBank",products: ["Transfer", "Withdrawal"],marketValue: 90,address: "Skybank Tower",ceo:"Mr Uche",country: "Nigeria",employees: ["JohnBosco", "Taiwo", "Shola"]){organization, products}}',
    // 		})
    // 		.expect(200)
    // 		.end((err, res) => {
    // 			// res will contain array with one organization
    ...
    // 			expect(res.body.data.createOrganization).toHaveProperty("ceo");
    // 			expect(res.body.data.createOrganization).toHaveProperty("marketValue");
    // 			done();
    // 		});
    // });
    ...
    // 		.post(BASE_URL)
    // 		.send({
    // 			query: 'mutation { deleteOrganization (_id:"5f71abbd5936151c29e9419c") {_id} }',
    // 		})
    // 		.expect(200)
    // 		.end((err, res) => {
    // 			// res will contain array with one organization
    ...
    it("Updates organizations", function (done) {
        request
            .post(BASE_URL)
            .send({
            query: 'mutation { updateOrganization (_id:"5f71abbd5936151c29e9419c", organization:"demo-org", marketValue:"100%") { _id , organization} }',
        })
            .expect(200)
            .end(function (err, res) {
            if (err)
